...
def test_splits_priority_groups_when_claim_units_differ() -> None:
    result = build_profile_priority_queue(
        [
            {"article_id": "A1", "sentence_id": "1", "reason_code": "PROFILE_NOT_FOUND"},
            {"article_id": "A2", "sentence_id": "1", "reason_code": "PROFILE_NOT_FOUND"},
        ...
        [
            _derived("A1", "1", indicator="수출액", calculation="DIRECT_VALUE", frequency="연", time="2024", unit="억달러"),
            _derived("A2", "1", indicator="수출액", calculation="DIRECT_VALUE", frequency="연", time="2024", unit="%"),
        ],
    )

    assert [(row["unit"], row["claim_count"]) for row in result] == [("%", 1), ("억달러", 1)]
