...
def semantic_match(
    ...
    minimum_score: float = 0.7, min_margin: float = 0.1,
) -> list[MatchResult]:
    ...
    return [
        MatchResult(
            candidate_tbl_id=candidate.tbl_id, semantic_score=round(score, 6),
...
def _score(claim: ClaimSchema, candidate: KosisCandidateSchema) -> float:
    ...
    label_score = max((_label_similarity(indicator, label) for indicator in indicators for label in labels), default=0.0)
    compatibility = 0.0
    if claim.unit and any(compatible_units(claim.unit, unit) for unit in candidate.unit_names): compatibility += 0.1
    if claim.frequency and claim.frequency in {part.strip() for part in (candidate.frequency or "").split("|")}: compatibility += 0.1
...
def _series_transformation_penalty(claim: ClaimSchema, candidate: KosisCandidateSchema) -> float:
    ...
    claim_text = _normalize(" ".join(value for value in [claim.source_sentence, claim.indicator, *(claim.dimension or {}).values()] if value))
    return 0.25 if "계절조정" in candidate_name and "계절조정" not in claim_text and "계절조절" not in claim_text else 0.0


def _unrequested_axis_count(claim: ClaimSchema, candidate: KosisCandidateSchema) -> int:
    requested_axes = len(claim.dimension or {})
    if claim.region and claim.region not in {"전국", "대한민국", "한국"}: requested_axes += 1
    return max(0, len(candidate.dimension_ids or candidate.dimension_names) - requested_axes)
...
def _indicator_variants(claim: ClaimSchema) -> set[str]:
    ...
    if indicator.endswith("액"): variants.add(f"{indicator[:-1]}금액")
    for value in (claim.dimension or {}).values():
        member = _normalize(value)
        residual = indicator.replace(member, "") if member else indicator
        if len(residual) >= 2: variants.add(residual)
    return variants


def _aggregate_scope_score(claim: ClaimSchema, candidate: KosisCandidateSchema) -> float:
    if claim.dimension or claim.population or claim.region not in {None, "전국", "대한민국", "한국"}: return 0.0
    return 0.15 if any(token in _normalize(candidate.tbl_name) for token in ("총괄", "총계", "전체")) else 0.0


def _label_similarity(indicator: str, label: str) -> float:
    if len(indicator) >= 2 and indicator in label: return 1.0
...
def _route(score: float, margin: float, minimum_score: float, min_margin: float) -> tuple[str, str]:
    if score < minimum_score: return "HOLD", "LOW_SEMANTIC_SCORE"
    if margin < min_margin: return "HOLD", "AMBIGUOUS_MARGIN"
