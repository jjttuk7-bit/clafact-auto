...
_FORECAST = re.compile(
    ...
    r"떨어질\s*것|오를\s*것|것으로\s*(?:분석|평가)|것[”\"’']?\s*이라고\s*(?:발표|했|말)|"
    r"달성(?:도)?\s*가능|목표(?:로|치)?)"
)
...
_POLICY_RATE = re.compile(
    r"(?:관세율|세율|관세).{0,20}\d+(?:\.\d+)?\s*(?:%|퍼센트)|"
    r"\d+(?:\.\d+)?\s*(?:%|퍼센트).{0,20}(?:관세율|세율|관세)"
)
_POLICY_THRESHOLD = re.compile(
    r"(?:지정\s*기준|적용\s*기준|상한(?:액)?|하한(?:액)?|기준\s*금액|"
    r"법인차.{0,30}표지판|해야\s*한다)"
)


def observation_preverification_reason(claim: ClaimSchema) -> str | None:
    """Return a deterministic reason for non-observed or non-statistical values."""
    ...
    if _POLICY_RATE.search(source):
        return "NON_STATISTICAL_POLICY_RATE"
    if _POLICY_THRESHOLD.search(fields) and not _OBSERVED.search(source):
        return "NON_STATISTICAL_POLICY_THRESHOLD"
    forecast = _FORECAST.search(source)
    if forecast is not None and _OBSERVED.search(source, forecast.end()) is None:
