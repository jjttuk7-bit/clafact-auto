...
def _unit_from_item_name(item_name: str) -> str:
    """Recover an explicit official unit suffix when KOSIS omits UNIT_NM."""
    match = re.search(
        r"\(\s*(%|％|명|천명|만명|건|개|호|가구|원|천원|만원|ha|㏊|헥타르)\s*\)\s*$",
        item_name,
        flags=re.IGNORECASE,
    )
...
def normalize_item_metadata(rows: Iterable[Mapping[str, object]], *, table_id: str) -> OfficialTableStructure:
    ...
    for row in rows:
        ...
        dimension_name = str(row.get("OBJ_NM", "")).strip()
        dimension_id = str(row.get("OBJ_ID", "")).strip()
        explicit_unit = str(row.get("UNIT_NM", "")).strip()
        unit = explicit_unit or (_unit_from_item_name(item_name) if dimension_id == "ITEM" else "")
        is_measurement_item = dimension_id == "ITEM" or bool(explicit_unit)
...
def hydrate_candidate(candidate: KosisCandidateSchema, official_structure: OfficialTableStructure) -> KosisCandidateSchema:
    """Apply verified KOSIS metadata without discarding Catalog API units."""
    ...
    return candidate.model_copy(update={
        ...
        "unit_names": list(official_structure.unit_names or candidate.unit_names),
        "item_units": dict(official_structure.item_units or candidate.item_units),
...
def hydrate_candidates_from_official_metadata(
    ...
    for candidate in candidates:
        try:
            ...
            hydrated.append(
                candidate if candidate.metadata_status == "OFFICIAL_METADATA_READY"
                else candidate.model_copy(update={"metadata_status": "OFFICIAL_ITEM_METADATA_UNAVAILABLE"})
            )
    return hydrated
